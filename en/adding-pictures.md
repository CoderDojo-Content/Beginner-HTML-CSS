...
3. Notice that the `<img>` tag is different from the other tags you've used so far: 
   ...
   * The tag contains two extra pieces of information inside it called **attributes**. The `src` attribute \(short for **source**\) tells the browser what file to use for the picture. If the browser cannot show the picture \(for example, if the file is not there\), it should display some text instead. The `alt` attribute \(called **alt text**\) is a short description that the browser will show if it cannot display the picture. 
